...
- [x] 12. Mandatory end-to-end test
  - `tests/e2e/test_spec001_mandatory_slice.py`: ONE clearly named mandatory scenario, `test_mandatory_slice_move_cube_50cm_right_end_to_end`, asserting all eleven required outcomes in order — 202 accepted, job succeeded, canonical ChatResponse success (schema-validated), saved `.blend` reopened by a FRESH Blender process, Cube X = 0.50 m within the Task 4 tolerance, preview artifact exists, project-scoped preview URL, HTTP 200, `image/png`, valid non-empty PNG whose SHA-256 matches its recorded checksum, and no filesystem path in any response OR in the PNG bytes.
  - Also asserts the AgentProvider/JobFactory output on the way through: `provider: rule_based`, `job_type: move_object`, `delta_meters {x:0.5,y:0,z:0}`, `origin {request_id, operation_index:0}`, and that metres remain canonical in the saved scene.
  - Failure case — worker/Blender unavailable, in two forms: no worker connected → 503 `BLENDER_UNAVAILABLE` with no job claimed and no artifact; worker connected but Blender unrunnable (through the real `BlenderOperationExecutor` boundary) → job `failed` with `BLENDER_UNAVAILABLE`, phase `failed`, no result, no preview.
  - Failure case — invalid object: the instruction is interpretable so it is ACCEPTED (202) and reaches execution, where Blender's own scene read produces `OBJECT_NOT_FOUND`; no preview artifact is created, `preview/latest` stays 404, and no object was invented.
  - Failure case — lock conflict: the project lock is held DETERMINISTICALLY through the existing Task 6 `FileLockProvider` (verified that `flock` conflicts across open file descriptions, so no sleep and no race window), producing the existing structured `LOCK_CONFLICT`. Nothing is journalled, no recovery copy or preview is made, and a follow-up test proves the lock is released rather than poisoning the project. No second locking mechanism was introduced.
  - No-corruption check shared by every failure case (`assert_project_intact`): the `.blend` exists and is non-empty, a fresh Blender process can open and read it, Cube is exactly where expected on all three axes, `object_id` survived, the scene still contains only `Cube`, and units are still METRIC/METERS.
  - Idempotency re-proven end to end: 0.00 → 0.50, verbatim retry of the same `request_id` resolves to the same job with `duplicate: true` and reuses the existing preview (still 0.50), a new `request_id` reaches 1.00 with a NEW artifact while the earlier artifact stays retrievable, and a stale retry after the second command leaves 1.00.
