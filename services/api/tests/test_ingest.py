...
# --- fixtures that build REAL files, not fakes ---------------------------


def png_bytes(width: int = 8, height: int = 6, colour: tuple[int, int, int] = (200, 40, 40)) -> bytes:
...
def pdf_bytes(pages: int = 2, text: str = "CEILING HEIGHT 2400") -> bytes:
    """A minimal but genuinely valid multi-page PDF carrying real text."""
    objects: list[bytes] = []
    ...
    for index in range(pages):
        content = (
            f"BT /F1 24 Tf 72 700 Td ({text} page {index + 1}) Tj ET".encode("ascii")
        )
        objects.append(
            (
