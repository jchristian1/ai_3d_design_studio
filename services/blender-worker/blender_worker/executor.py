...
Spec 001, Task 6. Connects the canonical Job (Task 3) to the retry-safe
MoveObjectPlan (Task 4) and the seed project (Task 5).

Orchestration only: no bpy, no fcntl, no filesystem layout decisions. Those live
behind ``BlenderOperationExecutor``, ``ProjectLockProvider``,
``WorkerExecutionStore`` and ``ProjectLocator`` so each can be replaced
independently.
